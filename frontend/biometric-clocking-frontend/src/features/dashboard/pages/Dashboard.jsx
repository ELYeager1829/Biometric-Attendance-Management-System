...
async function createLocalhostPasskey(employeeId) {
  const credential = await navigator.credentials.create({
    publicKey: {
      ...
      user: {
        ...
        name: `admin-${employeeId}@localhost`,
...
function DeleteModal({ employee, onClose, onRemove }) {
  ...
  const handleRemove = async () => {
    ...
    try {
      await scanFingerprint(employee.id)
...
function AdminRequestModal({ request, employee, onApprove, onClose }) {
  const [verifying, setVerifying] = useState(false)
  const [error, setError] = useState('')
  const action = employee?.status === 'Clocked In' ? 'clock out' : 'clock in'
  ...
  const handleApprove = async () => {
    ...
    try {
      await scanFingerprint(request.employeeNumber)   // same Windows Hello gate DeleteModal already uses
      await onApprove(request.employeeNumber, request.overrideRequestId)
    } catch (err) {
      setError(err.message || 'Fingerprint scan was not completed.')
      setVerifying(false)
    }
  ...
  return (
    <Modal onClose={onClose}>
      ...
      {error && <p className="mt-3 text-xs font-bold text-rose-300">{error}</p>}
      <button onClick={handleApprove} disabled={verifying} className="mt-6 w-full rounded-lg bg-sky-600 py-3 text-sm font-bold text-white disabled:opacity-40">{verifying ? 'Scanning fingerprint...' : `Clock them ${employee?.status === 'Clocked In' ? 'out' : 'in'}`}</button>
    </Modal>
  )
}
...
export default function Dashboard({ employees, pendingAdminRequest, onClearAdminRequest, onEmployeesChange, onEditEmployee, onLogout, onOnboard }) {
  const [query, setQuery] = useState('');
  const [action, setAction] = useState(null);
  const rows = useMemo(() => employees.filter(e => `${e.name} ${e.id}`.toLowerCase().includes(query.toLowerCase())), [employees, query]);
  const pendingEmployee = useMemo(() => employees.find(e => e.id.toLowerCase() === pendingAdminRequest?.employeeNumber?.toLowerCase()), [employees, pendingAdminRequest]);

  const approveAdminRequest = async (employeeNumber, overrideRequestId) => {
    const adminId = localStorage.getItem('userId')

    const response = await fetch(`/api/admin/override-requests/${overrideRequestId}/resolve`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ adminId })
    })

    if (!response.ok) throw new Error('Could not resolve the request. Please try again.')
    ...
    onEmployeesChange(currentEmployees => currentEmployees.map(employee => {
      ...
      return {
        ...employee,
        status: employee.status === 'Clocked In' ? 'Clocked Out' : 'Clocked In',
  ...
  return (
    <main className="flex min-h-screen bg-[#12304c] text-slate-200">
      ...
      <div className="min-w-0 flex-1 p-5 md:p-9">
        ...
        <Panel className="mt-6 overflow-hidden bg-[#173a5d] text-slate-200">
          ...
          <div className="flex gap-2 px-5 pb-4">
            ...
            <input id="dashboard-search" name="search" autoComplete="off" aria-label="Search name or employee ID" value={query} onChange={e => setQuery(e.target.value)} className="w-full bg-transparent px-2 py-2.5 text-xs outline-none" placeholder="Search name or employee ID..." />            </div>
          </div>
      ...
      {pendingAdminRequest && <AdminRequestModal request={pendingAdminRequest} employee={pendingEmployee} onApprove={approveAdminRequest} onClose={onClearAdminRequest} />}
    </main>
  )
}
