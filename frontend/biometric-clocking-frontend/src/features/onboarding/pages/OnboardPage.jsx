  ...
  const saveEmployee = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return   // stay on this page so the failure is visible, instead of navigating away
      }

      onBack()   // return to the directory once the request actually succeeded
    } catch (error) {
    ...
    <main className="min-h-screen bg-[#071525] text-slate-200">
      ...
      <section className="mx-auto max-w-5xl px-5 py-10">
        ...
        <footer className="mt-6 flex justify-end gap-3">
          ...
          <button onClick={saveEmployee} disabled={saving || !faceDescriptor} className="rounded-lg bg-sky-600 px-5 py-3 text-xs font-bold text-white hover:bg-slate-700 disabled:opacity-40">{saving ? 'Saving...' : (isEdit ? 'Save changes' : 'Register employee')}</button>
        </footer>
      </section>
    </main>
  )
