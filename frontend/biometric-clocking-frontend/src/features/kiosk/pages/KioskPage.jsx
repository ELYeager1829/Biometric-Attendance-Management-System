...
function AdminLogin({ onClose, onLogin }) {
  ...
  const loginRole = async () => {
    try {
      ...
      onLogin(
        email.trim().toLowerCase() === LOGIN_EMAILS.hr
          ? "hr"
          : "admin"
      );
...
function CallAdmin({ onClose, onNotify }) {
  ...
  const handleNotify = async () => {
    ...
    try {
      const response = await fetch('/api/admin/notify', {   // call the real backend
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ employeeNumber: trimmedNumber })
      })

      if (!response.ok) throw new Error('Notify request failed')   // treat non-2xx as failure, no fallback
      await response.json()   // confirm the body parses, even if unused

      onNotify(trimmedNumber)   // only called once the backend actually confirmed
      onClose()   // only close the modal on real success
    } catch {
      setError('Could not notify admin. Please try again.')   // show a real failure state
    } finally {
...
export default function KioskPage({ onAdminAccess, onAdminCall }) {
  ...
  useEffect(() => {
    ...
    const verifyFace = async () => {
      ...
      try {
        const response = await fetch('/api/face/verify', {   // call the .NET backend
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ descriptor }),


          // send the descriptor array as JSON

        })

        if (!response.ok) throw new Error('Face verify request failed')   // treat non-2xx as failure, no auto-success
        const result = await response.json()   // parse the backend's response

        if (result.matched) {
          setFaceStatus(`Welcome, ${result.name}`)   // show the matched employee's name
          setModal('face-success')
          setOtpChallengeId(result.otpChallengeId)  // only open on a real, confirmed match
        } else {
        ...
        setFaceStatus('Verification error. Retrying...')   // network/server error
        faceCheckStartedRef.current = false   // unlock so it retries on the next interval
      }
    }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await fetch('/api/otp/verify', {   // call the real backend
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ challengeId: otpChallengeId, code: otp })
      })

      if (!response.ok) throw new Error('OTP verify request failed')   // treat non-2xx as failure, no auto-success
      const result = await response.json()   // parse the backend's response
      ...
      setVerifying(false)   // stop the "checking" state
      setFaceStatus('Could not verify OTP. Please try again.')   // show a real failure, not a fake pass
    }
  }

  const handleResendOtp = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/otp/challenge`, {   // call the real backend to send a new code
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ employeeNumber: matchedEmployeeNumber, clockType: 'ClockIn' })
      })

      if (!response.ok) throw new Error('OTP resend failed')   // treat non-2xx as failure
      setModal('otp-valid')   // only show the "OTP is valid" notice on real success
    } catch {
      setModal(null)
      setFaceStatus('Could not send a new code. Please try again.')   // show a real failure instead of faking success
    }
  }
